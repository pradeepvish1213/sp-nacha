...
const highLevelOverrides = ['transactionCode', 'receivingDFI', 'checkDigit', 'DFIAccount', 'amount', 'adviceRoutingNumber', 'fileIdentification', 'achOperatorData', 'individualName', 'discretionaryData', 'addendaId', 'routingNumberOfAchOperator', 'julianDateOnWhichAdviceIsCreated', 'sequenceNumberWithinBatch'];

function EntryADV(options) {
    this._addendas = [];
    ...
    if (options.routingNumberOfAchOperator) {
        this.fields.routingNumberOfAchOperator.value = utils.computeCheckDigit(options.routingNumberOfAchOperator).slice(0, -1);
    }
...
EntryADV.prototype.getRecordCount = function () {
    return this._addendas.length + 1;
};

EntryADV.prototype.generateString = function (cb) {
    let self = this;
...
EntryADV.prototype._validate = function () {
...
EntryADV.prototype.get = function (category) {
...
EntryADV.prototype.set = function (category, value) {
